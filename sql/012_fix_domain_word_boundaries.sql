...
SET investigation_domain =
  CASE
    WHEN question ~* '(^|[^a-z])(vs\.?|versus|nba|nfl|nhl|mlb|counter-strike|esports|playoffs)([^a-z]|$)'
      ...
      OR question ~* '(^|[^a-z])(spread|moneyline)([^a-z]|$)'
      THEN 'SPORTS_ESPORTS'

    WHEN question ~* '(^|[^a-z])(war|wars|airstrike|airstrikes|missile|missiles|invasion|ceasefire|sanction|sanctions)([^a-z]|$)'
      OR question ~* '(^|[^a-z])(iran|israel|gaza|ukraine|russia|hormuz|houthi|houthis)([^a-z]|$)'
      OR question ~* '(military action|military operation|nuclear facility|kharg island|panama canal)'
      OR question ~* '(^|[^a-z])(strike|strikes|struck)([^a-z]|$)'
      THEN 'GEOPOLITICS'
